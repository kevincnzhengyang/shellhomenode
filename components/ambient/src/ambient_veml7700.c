...
static float veml7700_get_resolution(veml7700_handle_t dev);
/**
...
static esp_err_t veml7700_optimize_configuration(veml7700_handle_t dev, double *lux)
{
	if (ceil(*lux) >= veml7700_get_current_maximum_lux(dev)) {	// Decrease resolution
		...
		if (dev->configuration.maximum_lux == veml7700_get_maximum_lux(dev)) {
			ESP_LOGI(S_TAG, "Already configured for maximum luminocity.");
			return ESP_FAIL;
		}
		...
		if (dev->configuration.maximum_lux == veml7700_get_lowest_maximum_lux(dev)) {
			ESP_LOGI(S_TAG, "Already configured with maximum resolution.");
			return ESP_FAIL;
		}
		increase_resolution(dev);
	} else {
		ESP_LOGI(S_TAG, "Configuration already optimal.");
		return ESP_FAIL;
	}

	ESP_LOGI(S_TAG, "Configuration optimized.");
	return ESP_OK;
}
...
static void decrease_resolution(veml7700_handle_t dev)
{
	...
	ESP_LOGI(S_TAG, "Decreasing sensor resolution...");
	...
	if (gain_index == 3) {
		...
		if (resolution_map[it_index + 1][gain_index] > dev->configuration.resolution) {
			...
			ESP_LOGE(S_TAG, "Failed to decrease sensor resolution.");
		}
	}
...
esp_err_t veml7700_initialize(veml7700_handle_t *dev, int i2c_master_num)
{
    ...
    rdev->configuration.gain = VEML7700_GAIN_2;
    rdev->configuration.integration_time = VEML7700_IT_400MS;
...
esp_err_t veml7700_read_als_lux(veml7700_handle_t dev, double* lux)
{
	...
	if (i2c_result != 0) {
		ESP_LOGW(S_TAG, "veml7700_i2c_read() returned %d", i2c_result);
		return i2c_result;
	}
...
esp_err_t veml7700_read_als_lux_auto(veml7700_handle_t dev, double* lux)
{
	...
	ESP_LOGI(S_TAG, "Configured maximum luminocity: %"PRIu32"", dev->configuration.maximum_lux);
	ESP_LOGI(S_TAG, "Configured resolution: %0.4f", dev->configuration.resolution);
...
static veml7700_handle_t veml7700_dev;

#endif /* CONFIG_AMBIENT_VEML_7700 */

static Sensor_Ambient_Lisght *g_sensor = NULL;
...
static esp_err_t sensor_init(void * arg)
{
	...
	conf.sda_io_num = CONFIG_NODE_AMBIENT_LIGHT_SDA;
	conf.scl_io_num = CONFIG_NODE_AMBIENT_LIGHT_SCL;
	conf.sda_pullup_en = GPIO_PULLUP_ENABLE;
	conf.scl_pullup_en = GPIO_PULLUP_ENABLE;
	conf.master.clk_speed = CONFIG_NODE_AMBIENT_LIGHT_I2C_FREQUENCY;
	conf.clk_flags = 0;

    i2c_param_config(CONFIG_NODE_AMBIENT_LIGHT_I2C_NUM, &conf);
	i2c_driver_install(CONFIG_NODE_AMBIENT_LIGHT_I2C_NUM, I2C_MODE_MASTER, 0, 0, 0);
    ESP_LOGI(S_TAG, "i2c master setup");

    return veml7700_initialize(&veml7700_dev, CONFIG_NODE_AMBIENT_LIGHT_I2C_NUM);
#endif /* CONFIG_AMBIENT_VEML_7700 */
    return ESP_OK;
}
...
static esp_err_t sensor_read(float *o_lux)
{
    ...
    *o_lux = (float) lux_als;
    ESP_LOGI(S_TAG, "ALS %0.4f lux", *o_lux);
#endif /* CONFIG_AMBIENT_VEML_7700 */
    return ESP_OK;
}
